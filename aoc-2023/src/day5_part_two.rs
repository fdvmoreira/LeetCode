...
use std::{collections::HashMap, u32, u64, usize};
...
pub fn build_seeds(seeds: Vec<u32>) -> Option<Vec<u32>> {
    if seeds.is_empty() {
        return None;
    }
    ...
    seeds
        ...
        .for_each(|pair| {
            ...
            for idx in 0..*length as usize {
                new_seeds.push(start + idx as u32);
            }
        });
...
mod tests {
    ...
    #[test]
    fn test_build_seeds() {
        let input = vec![3, 4, 2, 10];
        let result = build_seeds(input).unwrap();
        assert_that!(result.len(), eq(14));
    }
}
